...
class EventPersistenceTest {
    ...
    private static Stream<Arguments> testRead() {
        return Stream.of(
                ...
                Arguments.of(Events.all(), ReadOptions.builder().until(Cursor.position(3)).build(), "a-0-0 a-0-1 b-1-0"),
                Arguments.of(Events.byLogId("inexistent"), ReadOptions.DEFAULT, ""),
                Arguments.of(Events.byContext("c"), ReadOptions.DEFAULT, "c-0-0 c-1-0"),
                Arguments.of(Events.byContext("c"), Direction.BACKWARD.readOptions().build(), "c-1-0 c-0-0"),
