...
public class Paginator {
    ...
    public Future<Long> forEach(AnyEvent.Handler<?> eventHandler) {
        ...
        Multi.createFrom().<AnyEvent>emitter(this::emitEvents)
                ...
                .subscribe().with(stats -> result.complete(stats.getCount()), result::fail);
        return result.future();
    }

    Future<Iterator<AnyEvent>> nextPage(final int numOfProcessedEvents, final Cursor currentPosition) {
    ...
    void emitEvents(MultiEmitter<? super AnyEvent> emitter) {
        ...
        Vertx.vertx().executeBlocking(result -> {
            Future<Iterator<AnyEvent>> currentPage = null;
            while (!emitter.isCancelled()) {
                if (currentPage != null && !currentPage.isComplete()) {
                    continue;
                }
                currentPage = nextPage(numOfProcessedEvents.get(), currentPosition.get())
